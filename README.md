...
1. Clone this repo to your local machine. Since you'll be using this as a starter for your project, you'll want to delete the `.git` directory.
    ```bash
    git clone git@github.com:ministryofjustice/wp-ppo.git .
    rm -rf .git
    ```
...
2. Create a `.env` file by copying from `.env.example`:
    ...
    Set the `SERVER_NAME` variable to `ppo.docker`. This is the hostname that will be used for development on your local machine.
...
6. Once the docker image has built and is running, you should be able to access the running container by going to `http://ppo.docker/` using your web browser.

    You will need to run through the WordPress installation wizard in your browser.

    The WordPress admin area will be accessible at `http://ppo.docker/wp/wp-admin`.
...
To see emails, go to http://mail.ppo.docker/ in your browser.
